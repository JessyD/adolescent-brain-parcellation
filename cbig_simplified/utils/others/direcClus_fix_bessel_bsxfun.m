...
for itcount = 1:noruns
    ...
    if lambdaFlag == 0
        ...
        while (ini_flag == 0)
            ini_loop = ini_loop + 1;
            [inilambda,~,exitflag] = fsolve(@(inputx) sign(inputx)*abs(besseli(alpha,inputx))-1e+10, start_value,optimset('Display','off'));
            if (exitflag == 1)
                lambda = inilambda;
                ini_flag = 1;
            else
...
if ((i == Inf)||(isnan(i)))
    ...
    [outNew fval exitflag]  = fsolve(@(argum) Ad(argum,D)-rbar,outu);
    if exitflag == 1
        out = outNew;
    else
        out = outu - D/(D-1)*rbar/2;
