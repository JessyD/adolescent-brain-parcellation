...
set cmd = "csh ${CBIG_CODE_DIR}/CBIG_fMRI_create_data_list.csh -sd ${sub_dir} -sub_ls ${sub_list} -folder surf -data_stem ${surf_stem}.nii.gz -out_dir ${scripts_dir} -out_stem surf${surf_stem} -preproc_opt ${preproc_opt}"
echo $cmd
eval $cmd

if( $scrub_flag == 1 ) then
	set cmd = "csh ${CBIG_CODE_DIR}/CBIG_fMRI_create_data_list.csh -sd ${sub_dir} -sub_ls ${sub_list} -folder qc -data_stem ${outlier_stem}.txt -out_dir ${scripts_dir} -out_stem outlier${outlier_stem} -preproc_opt ${preproc_opt}"
	echo $cmd
	eval $cmd
endif
